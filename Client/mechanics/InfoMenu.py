...
class InfoMenu(NinePatch):
    def __init__(self, world):
        NinePatch.__init__(self, world, [300, 0], 'images/test_text_box.9.png', [480, 340], layer=8)
        self.stage = MapObject(world, [320, 0], image='images/stage.png')
        self.level = Label(world, [385, 280], str(self.world.cur_player.level), 'Level', (82, 175, 46))
        self.cells = []
        for i in xrange(5):
            for j in xrange(4):
                if len(self.world.cur_player.items) > 4 * i + j:
                    self.cells.append(ImageButton(self.world, [520 + j * 63, 15 + i * 63], 'images/cell.9.png',
                                                  [58, 58],
    ...
    def draw_object(self):
        if self.is_focus:
            ...
            self.world.draw(self.world.cur_player.text_object.surface, [365, 235])
