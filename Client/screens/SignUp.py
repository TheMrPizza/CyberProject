...
class SignUp(Screen):
    def __init__(self, world):
        ...
        self.back = ImageButton(self.world, [1008, 20], 'images/elements/light_blue_color.9.png', [35, 35],
                                image='images/elements/white_arrow_right.png', square=28)

        self.username_label = Label(self.world, [410, 160], 'Username', fonts['Regular'], colors['light_blue'])
        self.username_status = MapObject(self.world, [500, 175], image='images/elements/light_green_v.png', square=16,
                                         is_visible=False)
        self.username_text_box = TextBox(self.world, [None, 193], 250, color=colors['light_blue'], middle=self.card)

        self.password_label = Label(self.world, [410, 245], 'Password', fonts['Regular'], colors['light_blue'])
        self.password_status = MapObject(self.world, [500, 175], image='images/elements/light_green_v.png', square=16,
                                         is_visible=False)
        self.password_text_box = TextBox(self.world, [None, 278], 250, color=colors['light_blue'], middle=self.card)

        self.pick = Label(self.world, [410, 330], 'Pick your color', fonts['Regular'], colors['light_blue'])
        self.colors = [ImageButton(self.world, [410 + 50*i, 370], 'images/elements/color_' + str(i+1) + '.9.png',
                                   [45, 45]) for i in xrange(5)]
        self.sign_up_button = TextButton(self.world, [None, 450], 'images/elements/dark_blue_box.9.png', [250, 45],
                                         text='Sign up', font=fonts['Medium'], color=colors['white'], middle=self.card)
        self.problem = Label(self.world, [None, 500], '', fonts['Small'], colors['light_red'], middle=self.card,
                             is_visible=False)
    ...
    def check_event(self, event, objects=None):
        if objects is None:
            objects = []
        Screen.check_event(self, event, self.colors + [self.back, self.username_text_box, self.password_text_box,
                                                       self.sign_up_button] + objects)

    def draw_screen(self, objects=None):
        if objects is None:
            objects = []
        Screen.draw_screen(self, self.colors + [self.card, self.title, self.back, self.username_label,
                                                self.username_status, self.username_text_box, self.password_label,
                                                self.password_status, self.password_text_box, self.pick,
                                                self.sign_up_button, self.problem] + objects)
    ...
    def on_click(self, map_object, event):
        if map_object is self.back:
            ...
            if len(data) < 6 or len(data) > 15:
                self.username_status = MapObject(self.world, [500, 172], image='images/elements/light_red_x.png',
                                                 square=16)
                self.username_text_box.change_background('images/elements/light_red_cell.9.png')
                self.problem = Label(self.world, [None, 495], 'Username must contain 6 - 15 characters', fonts['Small'],
                ...
                if len(data) < 6 or len(data) > 20:
                    ...
                    self.problem = Label(self.world, [None, 495], 'Password must contain 6 - 20 characters',
                                         fonts['Small'], colors['light_red'], middle=self.card)
                else:
                    ...
                    if self.chosen_color is None:
                        ...
                        from SignIn import SignIn
                        self.world.client.create_player(self.username_text_box.text, self.password_text_box.text,
                                                        101 + self.chosen_color)
                        self.world.cur_screen = SignIn(self.world)
